...
import { SporkIdpOidcStackExportsZod } from "./exports";
...
export = async () => {
	...
	return all([identityPoolOutput]).apply(([idpool]) => {
		const exported = {
			spork_idp_oidc_cognito: {
				pool: idpool,
			},
		} satisfies z.infer<typeof SporkIdpOidcStackExportsZod>;

		const validate = SporkIdpOidcStackExportsZod.safeParse(exported);
		if (!validate.success) {
