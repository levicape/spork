...
import {
	SporkIdpUsersCognitoDomain,
	SporkIdpUsersStackExportsZod,
} from "./exports";

const PACKAGE_NAME = "@levicape/spork";
const COGNITO_ROOT_DOMAIN = SporkIdpUsersCognitoDomain;
...
export = async () => {
	...
	const userpoolsOutput = all(Object.entries(pools)).apply((entries) =>
		Object.fromEntries(
	...
	return all([userpoolsOutput]).apply(([userpools]) => {
		const exported = {
			spork_idp_users_cognito: {
				operators: userpools.operators,
			},
		} satisfies z.infer<typeof SporkIdpUsersStackExportsZod>;

		const validate = SporkIdpUsersStackExportsZod.safeParse(exported);
		if (!validate.success) {
