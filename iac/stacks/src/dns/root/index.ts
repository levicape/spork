...
import { SporkDnsRootStackExportsZod } from "./exports";
...
export = async () => {
	...
	if (!domainName) {
		...
		return {
			spork_dns_root_route53: {},
			spork_dns_root_acm: {},
		} satisfies z.infer<typeof SporkDnsRootStackExportsZod>;
	}
	...
	return all([route53Output, acmOutput]).apply(([route53, acm]) => {
		const exported = {
			spork_dns_root_route53: route53,
			spork_dns_root_acm: acm,
		} satisfies z.infer<typeof SporkDnsRootStackExportsZod>;

		const validate = SporkDnsRootStackExportsZod.safeParse(exported);
		if (!validate.success) {
